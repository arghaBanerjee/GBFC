          ...
          >
            ...
            <div style={{ marginTop: '0.5rem' }}>
              <button
                ...
                style={{
                  marginRight: '0.5rem',
                  background: myLikedPostIds.has(post.id) ? '#16a34a' : undefined,
                  color: myLikedPostIds.has(post.id) ? 'white' : undefined,
                  border: myLikedPostIds.has(post.id) ? '#16a34a' : undefined,
                }}
              >
                {myLikedPostIds.has(post.id) ? 'Unlike' : 'Like'} ({post.likes_count})
              </button>
            ...
            {(commentingPostId === post.id || post.comments.length > 0) && (
              <div style={{ marginTop: '1rem', border: '1px solid #d1d5db', padding: '1rem', borderRadius: '0.5rem', background: '#f9fafb' }}>
                ...
                {post.comments.length > 0 && (
                  <div style={{ marginTop: commentingPostId === post.id ? '1rem' : '0' }}>
                    <strong>Comments</strong>
                    {post.comments.map((c) => (
                      <p key={c.id} style={{ margin: '0.5rem 0' }}>
                        <small><strong>{c.user_full_name}:</strong> {c.comment}</small>
                      </p>
                    ))}
                  </div>
                )}
